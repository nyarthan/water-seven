---
name: agent-guidance
description: Design and edit instructions consumed by agents. Use for skills, AGENTS.md, CLAUDE.md, context pointers, and progressively disclosed agent documentation.
---

Reference for writing any document an agent consumes — a skill, an `AGENTS.md` or `CLAUDE.md`, or a document reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable — the agent taking the same _process_ every run, not producing the same output.
