...
File managers, screenshots, and clipboard behavior remain platform-native initially. Helium's native profiles, groups, history, and application accounts remain application-owned mutable state. Water Seven manages Helium installation, default-handler declarations, user-overridable policy defaults, and mandatory Bitwarden and Dark Reader installation. Helium already ships uBlock Origin as a built-in component. On `baratie`, `helium-primary` and `helium-secondary` target the existing native profile directories while `verify-helium-profiles` verifies their presence; Water Seven does not inspect or mutate their contents. On macOS, extension enforcement uses a user-approved configuration profile, and changing the effective default browser requires separate interactive user approval: Water Seven guides both consent steps and verifies the live policy and LaunchServices handler rather than writing an ineffective `LSHandlers` preference.
