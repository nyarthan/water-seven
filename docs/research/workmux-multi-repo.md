...
_Initially researched 2026-09-15 from the official site and its linked repository only. The model analysis used [`raine/workmux` commit `b2834949e356be08163e4e05c7944ab3e7a09380`](https://github.com/raine/workmux/tree/b2834949e356be08163e4e05c7944ab3e7a09380), release [`v0.1.262`](https://github.com/raine/workmux/releases/tag/v0.1.262); the Water Seven integration and follow-up diagnosis use v0.1.263 as recorded below._
...
## Water Seven integration decision

_On 2026-09-16, the owner accepted the loss of the aggregate capability and chose a full replacement rather than the recommended wrapper. The integration was verified against upstream release [`v0.1.263`](https://github.com/raine/workmux/releases/tag/v0.1.263), commit [`b26022043336d9c18d6b03260fbbac2859e1ed08`](https://github.com/raine/workmux/tree/b26022043336d9c18d6b03260fbbac2859e1ed08)._

Water Seven therefore installs Workmux and Pi on shared workstations, configures one focused Pi pane plus one shell pane in tmux window mode, and maps `prefix+w` to `workmux dashboard`. New worktrees live at `~/Projects/worktrees/{project}/<handle>`, below Water Seven's `XDG_PROJECTS_DIR` convention. Workmux skills and a Water Seven-maintained variant of the upstream Pi status extension are installed declaratively, so `workmux setup` is not part of activation.

The old `~/dev/work/<work-item>/<repository>` hierarchy and its `.work-item.toml` manifests remain untouched as historical state. Workmux neither imports nor owns those directories. New work spanning repositories must use one Workmux worktree per repository and coordinate them through the installed skills or manually.

### Local fixes for v0.1.263

The pinned release has three integration limitations that Water Seven handles downstream:

- **Interactive setup has no configuration off-switch.** Normal `add`, `init`, and `list` commands can offer to install agent hooks and skills, with declined choices persisted in `~/.local/state/workmux/setup.json` ([wizard gate](https://github.com/raine/workmux/blob/b26022043336d9c18d6b03260fbbac2859e1ed08/src/cli.rs#L994-L1002), [setup state](https://github.com/raine/workmux/blob/b26022043336d9c18d6b03260fbbac2859e1ed08/src/agent_setup/mod.rs#L401-L454)). The release writes `declined_skills` but does not read it when deciding whether to prompt for skills ([skill prompt](https://github.com/raine/workmux/blob/b26022043336d9c18d6b03260fbbac2859e1ed08/src/agent_setup/mod.rs#L570-L610)). [`patches/workmux/declarative-setup.patch`](../../patches/workmux/declarative-setup.patch) adds `setup_wizard: false`; Water Seven declares that setting and removes obsolete wizard state during Home Manager activation.
- **The official Pi integration has no waiting state.** Workmux's [status support table](https://workmux.raine.dev/guide/status-tracking/#supported-agents) marks Pi's Waiting capability unavailable, and the bundled extension reports only `working` or `done` ([extension](https://github.com/raine/workmux/blob/b26022043336d9c18d6b03260fbbac2859e1ed08/resources/pi/extensions/workmux-status.ts)). [`native/pi/extensions/workmux-status.ts`](../../native/pi/extensions/workmux-status.ts) preserves the upstream registration and subagent behavior, adds exact waiting status for Pi's blocking `ui_prompt_start`/`ui_prompt_end` events, and uses a conservative fallback for ordinary assistant text: a settled final response ending in a question is considered waiting. The fallback can misclassify rhetorical questions and cannot recognize requests that do not end in a question mark; it is intentionally a workaround rather than a semantic guarantee.
- **`Ctrl-b` is interpreted as plain `b` in the dashboard.** The dashboard maps `KeyCode::Char('b')` without checking modifiers, so tmux's default prefix can open the base-branch picker ([keymap](https://github.com/raine/workmux/blob/b26022043336d9c18d6b03260fbbac2859e1ed08/src/command/dashboard/keymap.rs#L65-L68)). [`patches/workmux/ignore-control-b.patch`](../../patches/workmux/ignore-control-b.patch) restricts that action to unmodified `b` and adds regression coverage for both dashboard views.
...
**Research recommendation:** replace custom per-repository worktree/tmux mechanics with Workmux, but retain the custom CLI as the multi-repository aggregate and transaction coordinator. Removing that layer loses the exact capability in question. Water Seven's integration decision above intentionally accepts that trade-off and retires the custom CLI.
